...
import { uploadClass, uploadJar } from '@/api/uploadApi'
import { Upload, FileArchive, FileCode2, CheckCircle2, AlertCircle } from 'lucide-react'

export function UploadForm() {
  const [file, setFile] = useState<File | null>(null)
  const [project, setProject] = useState('')
  const [version, setVersion] = useState('')
  const [type, setType] = useState<'class' | 'jar'>('class')
  ...
  return (
    <form onSubmit={handleUpload} className="space-y-6 max-w-md mx-auto">
      ...
      >
        ...
        <div className="text-sm mb-1">Drag & drop or click to select a {type === 'class' ? '.class' : '.jar'} file</div>
        <input ref={inputRef} type="file" accept={type==='class'?'.class':'.jar'} className="hidden" onChange={handleFileChange}/>
