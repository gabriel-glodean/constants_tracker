...
import static org.glodean.constants.model.UnitConstant.CoreSemanticType.UNKNOWN;
...
class ClassModelExtractorTest {
   ...
   */
  private ConstantUsage usage(UnitConstant.UsageType usageType) {
  ...
  /**
   * Creates a UnitConstant with the given value and usage types.
   * Helper method to simplify test assertions.
   *
   * @param value the constant value
   * @param usageTypes the structural usage types
   * @return a UnitConstant with the specified value and usages
   */
  private UnitConstant constant(Object value, UnitConstant.UsageType... usageTypes) {
    Set<ConstantUsage> usages = new HashSet<>();
    for (var usageType : usageTypes) {
      usages.add(usage(usageType));
    }
    return new UnitConstant(value, usages);
  }

  /**
   * Extracts constants from a class and returns the first UnitConstants result.
   *
   * @param classModel the class model to analyze
   * @return the extracted UnitConstants
   * @throws IOException if extraction fails
   */
  private UnitConstants extractConstants(ClassModel classModel) throws IOException {
    // build a UnitDescriptor using the class name as the path so tests can assert on it
    var className = Utils.toJavaName(classModel.thisClass().asSymbol());
    var results = new ClassModelExtractor(classModel, analysisMerger)
        .extract(new UnitDescriptor(BytecodeSourceKind.CLASS_FILE, className));
   ...
   * @throws IOException if extraction fails
   */
  private UnitConstants extractConstants(Class<?> clazz) throws IOException {
   ...
   *
   * @param actual the actual UnitConstants
  ...
  private void assertExtractedConstants(
      UnitConstants actual, String expectedClassName, Set<UnitConstant> expectedConstants) {
    // UnitConstants.source().path() contains the original class name we set in the UnitDescriptor
    assertEquals(expectedClassName, actual.source().path(), "Class name mismatch");
    ...
    // For each expected constant, verify it exists in actual with matching usage types
      for (UnitConstant expectedConstant : expectedConstants) {
      ...
      // Extract structural usage types from both
        Set<UnitConstant.UsageType> expectedUsageTypes = expectedConstant.usages().stream()
          .map(ConstantUsage::structuralType)
          .collect(java.util.stream.Collectors.toSet());
        Set<UnitConstant.UsageType> actualUsageTypes = matchingActual.usages().stream()
          .map(ConstantUsage::structuralType)
          .collect(java.util.stream.Collectors.toSet());
