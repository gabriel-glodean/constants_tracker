  ...
  public void handle(ArrayLoadInstruction instruction, State state, String tag) {
    ...
    if (arr != null)
      for (StackAndParameterEntity o : arr) {
        ...
        if (els != null) {
          res.addAll(els);
        } else if (o instanceof ObjectReference ref) {
          var componentType = ref.descriptor().componentType();
          if (componentType != null) {
            res.add(StackAndParameterEntity.convert(componentType, tag));
          }
        }
        // Non-ObjectReference entities (e.g. ObjectConstant from LDC) in the
        // array slot are silently skipped – we have no component-type info.
      }
