...
import java.lang.classfile.*; // ClassFile, ClassModel, MethodModel, CodeModel, etc.
import java.lang.classfile.instruction.*; // instruction subtypes
...
final class ByteCodeMethodAnalyzer {
    ...
    ByteCodeMethodAnalyzer(ClassModel cm, MethodModel mm) {
        this.cm = cm;
        this.methodModel = mm;
        this.methodTag = cm.thisClass().asInternalName() + "::" + mm.methodName().stringValue() + mm.methodType().stringValue();
        var attributes = mm.findAttribute(Attributes.code());
        this.maxLocals = attributes.map(CodeAttribute::maxLocals).orElse(0);
        CodeModel codeModel = mm.elementStream().filter(e -> e instanceof CodeModel).map(e -> (CodeModel) e).findFirst().orElse(null);
        if (codeModel == null) {
            this.code = List.of();
            return;
        }
    ...
    public void run() {
        ...
        if (!methodModel.flags().has(AccessFlag.STATIC)) {
            entry.locals.set(0,
                    PointsToSet.of(
                            new ObjectReference(cm.thisClass().asSymbol(),
                                    cm.thisClass().asInternalName() + "::<this>")));
            index++;
        }

        String tag = "Param";
        for (var paramType : methodModel.methodTypeSymbol().parameterList()) {
            entry.locals.set(index, PointsToSet.of(StackAndParameterEntity.convert(paramType, tag+"#"+index)));
            index++;
        }
    ...
    private State transfer(int i, State st) {
        ...
        switch (e) {
            case NewObjectInstruction ni -> PROVIDER.handlerFor(NewObjectInstruction.class).handle(ni, st, tag);
            ...
            case ConstantInstruction ci -> PROVIDER.handlerFor(ConstantInstruction.class).handle(ci, st, tag);
            ...
            case ArrayLoadInstruction ali -> PROVIDER.handlerFor(ArrayLoadInstruction.class).handle(ali, st, tag);
            case ArrayStoreInstruction asi -> PROVIDER.handlerFor(ArrayStoreInstruction.class).handle(asi, st, tag);
            case TypeCheckInstruction tc -> PROVIDER.handlerFor(TypeCheckInstruction.class).handle(tc, st, tag);
            ...
            case InvokeInstruction ii -> {
                ...
                calls.add(methodTag + " -> " + ii.owner().asInternalName() + "." + ii.name().stringValue() +
                        ii.type().stringValue());
            }
            case StackInstruction si -> PROVIDER.handlerFor(StackInstruction.class).handle(si, st, tag);
            case OperatorInstruction oi -> PROVIDER.handlerFor(OperatorInstruction.class).handle(oi, st, tag);
            case BranchInstruction bi -> PROVIDER.handlerFor(BranchInstruction.class).handle(bi, st, tag);
            case IncrementInstruction ii -> PROVIDER.handlerFor(IncrementInstruction.class).handle(ii, st, tag);
            default -> {
            }
        }
    ...
    String stringify(State s) {
        StringBuilder sb = new StringBuilder();
        sb.append("Locals:");
        for (int i = 0; i < s.locals.size(); i++) sb.append(" ").append(i).append("=").append(pt(s.locals.get(i)));
        sb.append(" | Stack:");
    ...
    String pt(PointsToSet s) {
        return s == null ? "-" : s.stream().map(Object::toString).sorted().collect(Collectors.joining("|"));
    }
}
