...
final class SuccessorBuilder {
     ...
     * catch types
     * @throws IllegalArgumentException if any label in the code cannot be resolved, but this should not happen as list of instructions can be obtained from a valid CodeModel.
     */
     ...
     * <p>
     * Note: In well-formed input produced by a valid CodeModel / class file every referenced
     * {@code Label} is bound to a corresponding {@code LabelTarget} present in the {@code code}
     * list. Therefore the thrown {@link IllegalArgumentException} is defensive and documents a
     * violated invariant; it should be impossible for normal operation and helps fail fast when
     * the builder is used incorrectly or receives malformed input.
     *
     ...
     * @throws IllegalArgumentException if no matching LabelTarget is found (indicates a programming error)
     */
