...
class AnnotationConstantExtractorTest {
  ...
  void extractAnnotationConstants() throws IOException {
    ...
    ClassConstants classConstants = results.iterator().next();

    assertEquals("com.example.AnnotatedController", classConstants.name());

    // Collect all constant values
    Set<Object> values = classConstants.constants().stream()
        .map(ClassConstant::value)
    ...
    assertSemanticType(classConstants, "users", CoreSemanticType.SQL_FRAGMENT);
    assertSemanticType(classConstants, "${mail.from}", CoreSemanticType.PROPERTY_KEY);
    assertSemanticType(classConstants, "/api/users", CoreSemanticType.API_ENDPOINT);
    assertSemanticType(classConstants, "\\d+", CoreSemanticType.REGEX_PATTERN);
    assertSemanticType(classConstants, "hello", CoreSemanticType.UNKNOWN);
  }
  ...
  private void assertSemanticType(
      ClassConstants classConstants, Object value, CoreSemanticType expected) {
    ...
    boolean found = constant.usages().stream()
        .anyMatch(u -> u.structuralType() == UsageType.ANNOTATION_VALUE
